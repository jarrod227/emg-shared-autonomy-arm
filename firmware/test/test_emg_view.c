...
{
    CHECK(emg_view_activation(0, 100) == 0);
    CHECK(emg_view_activation(99, 100) == 0);
    CHECK(emg_view_activation(100, 100) == 0);
    CHECK(emg_view_activation(101, 100) > 0);
    /* A threshold of zero would divide by a zero span. */
    CHECK(emg_view_activation(500, 0) == 0);
    CHECK(emg_view_activation(500, -10) == 0);
}
...
static void test_saturates_rather_than_wrapping(void)
{
    ...
    CHECK(emg_view_activation(reference, threshold) == 65535);
    CHECK(emg_view_activation(reference * 10, threshold) == 65535);
    /* The largest total the features can produce must not wrap either. */
    CHECK(emg_view_activation(3 * 32767, threshold) == 65535);
}
...
static void test_monotonic_and_multiplies_before_dividing(void)
{
    ...
    for (int32_t total = 1000; total <= 3000; total++) {
        const uint16_t value = emg_view_activation(total, threshold);
    ...
    for (int32_t i = 0; i < count; i++) {
        for (int32_t step = 0; step < steps; step++) {
            ...
            const uint16_t value =
                emg_view_activation(total, thresholds[i]);
